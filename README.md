...
```mermaid
flowchart TD
    UI[Kullanıcı Arayüzü / Çözüm Tahtası] --> Telemetry[Zaman Damgalı Adım ve Telemetri]
    Telemetry --> CAS[Deterministik Sembolik Doğrulayıcı - SymPy AST]
    CAS --> BugEngine[Hata ve Buggy Rule Sınıflandırıcı]
    BugEngine --> LearnerModel[5D Yaşayan Öğrenici Modeli - iBKT / DDM / Tip-2 SDT]
    LearnerModel --> AdaptiveEngine[Adaptif Öğretim Motoru & ZPD Termostatı]
    AdaptiveEngine --> TutorAgent[AI Tutor LLM Orkestratörü]
    TutorAgent --> Guardrail[Zero-Leakage Regex Güvenlik Sübabı]
    Guardrail --> UI
```
